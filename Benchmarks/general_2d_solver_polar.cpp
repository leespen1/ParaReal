 ...
 *     u'(t, u) = Au
 * , where u is a real 2D vector and A is a real 2x2 matrix 
 *
 ...
 * different matrix by providing an additional 2 arguments on the command line
 * (magnitude and angle).
 */
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 7 && argc != 9) {
        if (my_rank == ROOT)
            cout << "Usage: program_name num_fine_solve num_runs [x1 x2 <2 entries of initial value>] [t0 tf <start and end time>] [r theta <magnitude and angle, as a multiple of pi>]\n";
        MPI_Finalize();
        return 0;
    }
    ...
    if (argc == 9) {
        r = atof(argv[7]);
        theta = atof(argv[8]);
        double angle = PI*theta;

        g_mat_init[0][0] = r*cos(angle);
        g_mat_init[0][1] = -r*sin(angle);
        g_mat_init[1][0] = r*sin(angle);
        g_mat_init[1][1] = r*cos(angle);
    }
