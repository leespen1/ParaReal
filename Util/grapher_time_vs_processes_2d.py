...
fig.suptitle("Time Taken By ParaReal Implementation")
fig.supxlabel(r"$\theta/\pi$")
fig.supylabel("Avg Time to Converge (s)")
...
for i in file_numbers:
    ...
    ax[0, 0].axhline(y=serial_times_dict[i], color=color, label="Serial Time - tf = %s"%i)
...
for n, i in enumerate(file_numbers):
    ...
    ax[(1+n)//2, (1+n)%2].axhline(y=serial_times_dict[i], color=color, marker=marker)
    ax[(1+n)//2, (1+n)%2].set_title("$t_f$ = %s"%(i))
    ax[(1+n)//2, (1+n)%2].set_xticklabels(['1', '5/6', '3/4', '2/3', '1/2'])

fig.legend(loc=2, prop={'size': 6}, ncol=2)
plt.tight_layout()
plt.show()
